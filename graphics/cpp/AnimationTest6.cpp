...
Model *AnimationTest6::create()
{
    ...
    if (!modelInfo)
    {
        fatalError("Failed to create ModelInfo object: animation_test_6");
    }
    modelInfo->setFileName("animation_test_6");
    ...
    ShaderInfo *vertexShaderInfo = vertexShaderInfo = new ShaderInfo(ShaderInfo::VERTEX_SHADER, 
                ShaderInfo::TEXT_FILE, std::string("shaders/anim_test_6.cgvert"), 
                model->getVertexType(), "anim_test_6_context");
    ShaderInfo *fragmentShaderInfo = new ShaderInfo(ShaderInfo::FRAGMENT_SHADER, 
        ShaderInfo::TEXT_FILE, std::string("shaders/anim_test_6.cgfrag"), 
        model->getVertexType(), "anim_test_6_context");
...
void AnimationTest6::initialize()
{
    totalTime = 2.0f;
    ...
    offsetTransforms = new float[numberOfBones * 16];
...
void AnimationTest6::update(float deltaSeconds)
{
    if (!wasInitialized)
    {
        initialize();
...
void AnimationTest6::setCurrentAnimationTime(float time)
{
    vertexShader->setUniformParameter("currentTime", &time);
}

void AnimationTest6::setTotalAnimationTime(float time)
{
    vertexShader->setUniformParameter("totalTime", &time);
}

void AnimationTest6::draw(Graphics *graphics, bool picking)
{
    if (picking)
        currentTime = 0.0f;
